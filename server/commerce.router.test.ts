...
describe("commerce.products", () => {
  ...
  it("filters, paginates, and counts catalog products on the server", async () => {
    ok({
      products: {
        edges: [
          { node: { ...rawProduct, id: "gid://shopify/Product/1", handle: "aria", title: "Cordless Drill", productType: "Tools & Equipment", tags: ["department:tools-equipment", "modality:physical-goods"], priceRange: { minVariantPrice: { amount: "2999.00", currencyCode: "ZAR" }, maxVariantPrice: { amount: "2999.00", currencyCode: "ZAR" } } } },
          { node: { ...rawProduct, id: "gid://shopify/Product/2", handle: "solar", title: "Solar Site Light", productType: "Energy & Power Solutions", tags: ["department:energy-power", "modality:physical-goods"], priceRange: { minVariantPrice: { amount: "3899.00", currencyCode: "ZAR" }, maxVariantPrice: { amount: "3899.00", currencyCode: "ZAR" } } } },
        ],
      },
    });
    const caller = appRouter.createCaller(makeCtx());
    const result = await caller.commerce.products.search({ q: "drill", modality: "all", sort: "price_desc", availableOnly: false, limit: 24, offset: 0 });
    expect(result.total).toBe(1);
    expect(result.items[0].handle).toBe("aria");
    expect(result.counts.departments).toMatchObject({ "tools-equipment": 1, "energy-power": 1 });
