  ...
  return <ApplicationShell role={user.userType === "enterprise" ? "enterprise" : "employer"}>
    ...
    <div className="mx-auto max-w-3xl">
      ...
      <section className="rounded-2xl border border-border bg-card p-6 md:p-8">
        {step === 0 && <div className="space-y-5"><div><h2 className="text-xl font-semibold">Start with the outcome</h2><p className="mt-1 text-sm text-muted-foreground">A specific title and trade category improve the quality of applications.</p></div><div><Label htmlFor="job-title">Job title</Label><Input id="job-title" value={form.title} onChange={(event) => update("title", event.target.value)} placeholder="e.g. Electrical rewiring for a three-bedroom home" className="mt-1.5" maxLength={200} /></div><div><Label htmlFor="job-vocation">Vocation</Label><Select value={form.vocation} onValueChange={(value) => update("vocation", value)}><SelectTrigger id="job-vocation" className="mt-1.5"><SelectValue placeholder="Select a vocation" /></SelectTrigger><SelectContent>{VOCATION_KEYS.map((key) => <SelectItem key={key} value={key}>{VOCATION_LABELS[key as VocationKey]}</SelectItem>)}</SelectContent></Select></div></div>}
